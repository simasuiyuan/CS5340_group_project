...
def _discretize(df_rt, num_of_cls):
    ...
    GMM_cluster = GMM(num_of_cls, covariance_type='full', random_state=0).fit(Y)
    ...
    metadata = {'mean': dict(enumerate(GMM_cluster.means_.flatten()))}
    metadata['discretization_model'] = GMM_cluster
    metadata['num_of_cls'] = num_of_cls
