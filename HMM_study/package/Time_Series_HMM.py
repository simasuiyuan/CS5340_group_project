...
#             self._logger.info(f'>>> Sampling from {feature}')
...
#         self._logger.info(f'>>> hist data date index from {previous_data_start_index} to {previous_data_end_index}')
#         self._logger.info(f'>>> predict data date index {day_index}')
...
#             self._train_data = self.data[:(len(self._train_data)+1)]
#             self._test_data = self._test_data[1:]
            
        if with_plot:
            test_data = self._test_data[0: days]
            #days = test_data.index
