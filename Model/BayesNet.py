...
class BNModel(ModelInterface):
    ...
    def project(self, projection_data, **kwargs):
        ...
        for col in self.inference_data.columns:
            if not (col in self.model['model'].nodes):
                del self.inference_data[col]
        
        cfg_proj =  {'func': projection.project, 'params': {'data': self.inference_data, 
                                        'metadata': self.metadata, 'model': self.model['model'], 'mode': 'weighted'}}
